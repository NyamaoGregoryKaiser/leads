    ...
    try:
        if 'GOOGLE_CREDENTIALS' in os.environ:
            ...
            if os.path.exists('credentials.json'):
                ...
                st.warning("⚠️ No credentials found! Using sample data for testing.")
        ...
        # Handle duplicate column names by consolidating AGENT NAME columns
        processed_headers = []
        agent_name_columns = []
        non_agent_columns = []
        ...
        for i, header in enumerate(headers):
            if header == 'AGENT NAME':
                agent_name_columns.append(i)
            else:
                non_agent_columns.append(i)
                processed_headers.append(header)
        
        # Add single AGENT NAME column
        processed_headers.append('AGENT NAME')
        
        # Get data rows (skip header)
        data_rows = all_values[1:]
        
        # Process data rows to consolidate AGENT NAME columns
        processed_data = []
        for row in data_rows:
            processed_row = []
            agent_names = []
            
            # Add non-agent columns first
            for i in non_agent_columns:
                if i < len(row):
                    processed_row.append(row[i])
                else:
                    processed_row.append('')
            
            # Collect agent names from all AGENT NAME columns
            for i in agent_name_columns:
                if i < len(row) and row[i].strip():
                    agent_names.append(row[i].strip())
            
            # Add consolidated agent names
            if agent_names:
                processed_row.append(', '.join(agent_names))
            else:
                processed_row.append('')
        ...
        st.warning(f"⚠️ Error connecting to Google Sheets: {str(e)}")
        st.info("Using sample data for testing. Please check your credentials and internet connection.")
        # Return sample data for testing
        return create_sample_data()

def create_sample_data():
    """Create sample data for testing when Google Sheets is not available"""
    import random
    from datetime import datetime, timedelta
    
    # Sample data structure
    branches = ['Nairobi', 'Mombasa', 'Kisumu', 'Nakuru', 'Eldoret']
    sources = ['Website', 'Social Media', 'Referral', 'Cold Call', 'Event']
    team_leaders = ['CECILIA NAISORA MACKSON', 'Chris Konzi Mbinda', 'Cosmas Gathogo Njogu', 
                   'ERIC OWUOR', 'George Barasa Barasa', 'GRACE WANGARI KARIUKI']
    ...
    sample_data = []
    for i in range(500):  # 500 sample records
        date = start_date + timedelta(days=random.randint(0, 90))
        sample_data.append({
            'DATE': date.strftime('%Y-%m-%d'),
            'BRANCH': random.choice(branches),
            'SOURCE OF LEAD GENERATION': random.choice(sources),
            'TEAM LEADER NAME': random.choice(team_leaders),
            'AGENT NAME': random.choice(agents),
            'LOCATION': random.choice(locations),
            'CLIENT CONTACT': f'07{random.randint(10000000, 99999999)}'
        })
